...
ggplot(df, aes(x = Date, y = Volume)) +
  ...
  labs(title = "Volume de Negociação da NVIDIA",
       x = "Data", y = "Volume") +
  theme_bw()
```
...
Para as quatro variáveis de preço, as distruições são assimétricas positivas. É observado alguns outliers, explicados pela alta que houve nos anos anteriores.
...
#### Gráfico de decomposição para o fechamento
...
data_summary <- df_heatmap %>%
  group_by(Year,Month) %>%
  summarise(Average_Close = mean(Close.Last, na.rm = TRUE), .groups = "drop") %>%
  ungroup()

ggplot(data_summary, aes(x = Year, y = Month, fill = Average_Close)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "yellow", high = "red") +
  labs(title = "Heatmap de Sazonalidade",
       x = "Ano",
       y = "Mês",
       fill = "Preço Médio") +
  theme_bw()
